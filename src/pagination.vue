<template>
  <ul class="pagination">
    ...
    <li v-if="Math.max(this.currentPage - this.maxNumberOnSide, 1) === this.maxNumberOnSide"  :class="{active: currentPage === 1}" >
      <a :href="url + '?page=1'" @click.prevent="gotoPage(1)" class="active" data-page="1">1</a>
    </li>
    <li v-if="Math.max(this.currentPage - this.maxNumberOnSide, 1) > this.maxNumberOnSide"><a @click.prevent="">...</a></li>

    <li v-for="i in this.pagesInMiddle"  :class="{active: currentPage === i}" >
      <a :href="url + '?page=' + i" @click.prevent="gotoPage(i)">{{ i }}</a>
    </li>

    <li v-if="Math.min(this.currentPage + this.maxNumberOnSide, this.lastPage) < this.lastPage - 1"><a @click.prevent="">...</a></li>
    <li v-if="Math.min(this.currentPage + this.maxNumberOnSide, this.lastPage) < this.lastPage" :class="{active: currentPage === lastPage}">
...
<script >
  ...
  export default {
    ...
    computed: {
      pagesInMiddle () {
        let start = Math.max(this.currentPage - this.maxNumberOnSide, 1);
        let end = Math.min(this.currentPage + this.maxNumberOnSide, this.lastPage);
